...
class SoftmaxLoss(Module):
    def forward(self, logits: Tensor, y: Tensor) -> Tensor:
        ...
        return (ops.summation(ops.logsumexp(logits, axes=(1,)) - ops.summation(logits * y_one_hot, axes=(1,))) / batch_size)
        ### END YOUR SOLUTION
...
class BatchNorm1d(Module):
    def __init__(self, dim: int, eps: float = 1e-5, momentum: float = 0.1, device: Any | None = None, dtype: str = "float32") -> None:
        ...
        ### BEGIN YOUR SOLUTION
        self.weight = Parameter(init.ones(dim, device=device, dtype=dtype), dtype="float32")
        self.bias = Parameter(init.zeros(dim, device=device, dtype=dtype), dtype="float32")
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            mean_broadcasted = ops.broadcast_to(ops.reshape(mean, (1, self.dim)), x.shape)

            var = ops.summation((x-mean_broadcasted) ** 2, axes=(0,)) / x.shape[0]
            var_broadcasted = ops.broadcast_to(ops.reshape(var, (1, self.dim)), x.shape)
            ...
            x_normalized = (x - mean_broadcasted) / ops.power_scalar(var_broadcasted + self.eps, 0.5)
        else:
            mean_broadcasted = ops.broadcast_to(ops.reshape(self.running_mean, (1, self.dim)), x.shape)
            var_broadcasted = ops.broadcast_to(ops.reshape(self.running_var, (1, self.dim)), x.shape)
...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        mean_broadcasted = ops.broadcast_to(ops.reshape(mean, (x.shape[0], 1)), x.shape)

        var = ops.summation((x-mean_broadcasted) ** 2, axes=(1,)) / self.dim
        var_broadcasted = ops.broadcast_to(ops.reshape(var, (x.shape[0], 1)), x.shape)
