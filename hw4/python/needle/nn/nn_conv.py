...
class Conv(Module):
    ...
    def forward(self, x: Tensor) -> Tensor: # (N, C, H, W)
        ...
        if self.bias:
            # Reshape bias from (C_out,) to (1, 1, 1, C_out) to make it broadcastable to (N, H, W, C_out)
            N, H, W, C = out.shape
            bias_reshaped = self.bias.reshape((1, 1, 1, C))
            out = out + bias_reshaped.broadcast_to(out.shape)
