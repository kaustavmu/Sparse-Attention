...
class Transformer(Module):
    def __init__(
        ...
        *,
        ...
        # Add output projection from embedding_size to hidden_size
    ...
    def forward(self, x, h=None):
        ...
        if self.output_projection is not None:
            # x is (bs, seq_len, embedding_size)
            x_reshaped = x.reshape((batch_size * seq_len, self.embedding_size))
            x_projected = self.output_projection(x_reshaped)
            x = x_projected.reshape((batch_size, seq_len, self.hidden_size))
        ### END YOUR SOLUTION

        if not self.batch_first:
            x = ops.transpose(x, axes=(0, 1))  # (seq_len, bs, hidden_size)
