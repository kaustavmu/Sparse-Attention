...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        if out_grad.shape != a.shape:
            ...
            for axis in reversed(sorted(summed_dims)):  # reverse order to maintain correct indices
                out_grad = out_grad.sum(axis)
...
class Summation(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        if self.axes is None:
            # Sum over all axes
            restored_shape = tuple(1 for _ in a.shape)
        else:
...
class Tanh(TensorOp):
    ...
    def gradient(self, out_grad, node):
      ### BEGIN YOUR SOLUTION
      a = node.inputs[0]
      one = Tensor(array_api.full(out_grad.shape, 1.0), device=out_grad.device, dtype=out_grad.dtype)   
...
class Dilate(TensorOp):
    ...
    def compute(self, a):
        ...
        for axis in self.axes:
            # Only dilate if axis is within bounds
            if axis < len(a.shape):
...
class UnDilate(TensorOp):
    ...
    def compute(self, a):
        ...
        for axis in self.axes:
            # Only undilate if axis is within bounds
            if axis < len(a.shape):
                idxs[axis] = slice(None, None, 1 + self.dilation)
