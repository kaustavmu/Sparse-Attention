...
class NDArray:
    ...
    #     if old_size != new_size or not self.is_compact():
    #         raise ValueError()

    #     curr_stride = self.strides[-1]

    #     new_strides_reverse = []

    #     for i in new_shape[::-1]:
    #         new_strides_reverse.append(curr_stride)
    #         curr_stride *= i
            
    #     return self.as_strided(new_shape, tuple(new_strides_reverse[::-1]))
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for i, slc in enumerate(slices):
            new_shape.append((slc.stop - slc.start + slc.step - 1) // slc.step)
            new_strides.append(old_strides[i] * slc.step)
            new_offset += slc.start * old_strides[i]
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        for axis in axes:
            # Flip by reversing the stride direction and adjusting offset
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = tuple(
            self.shape[i] + left + right for i, (left, right) in enumerate(axes)
        )
        ...
        # Calculate slice indices for where to place original data
        slices = tuple(
            slice(left, self.shape[i] + left) for i, (left, right) in enumerate(axes)
        )
