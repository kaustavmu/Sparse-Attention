...
def softmax_loss(Z, y_one_hot):
    ...
    return np.log(np.sum(np.exp(Z), axis=1)).mean() - Z[np.arange(Z.shape[0]), y].mean()
    ### END YOUR SOLUTION
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for start in range(0, num_examples, batch):
        ...
        I = np.zeros((Z1.shape[0], W2.shape[1]))
        I[np.arange(end - start), y_batch] = 1
        G2 = np.exp(Z1 @ W2) / np.sum(np.exp(Z1 @ W2), axis=1, keepdims=True) - I
        G1 = (Z1 > 0).astype(np.float32) * (G2 @ W2.T)  # Gradient of ReLU
