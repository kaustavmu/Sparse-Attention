...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  int64_t offset_signed = static_cast<int64_t>(offset);
  const int64_t a_size = static_cast<int64_t>(a.size); // assuming a.size fits in int64

  for (size_t i = 0; i < out->size; ++i) {
    // compute the offset in a using signed 64-bit arithmetic
    int64_t a_offset = offset_signed;
    for (size_t d = 0; d < ndim; ++d) {
      a_offset += static_cast<int64_t>(indices[d]) *
                  static_cast<int64_t>(strides[d]);
    }

    // sanity check (optional but good for debugging)
    assert(a_offset >= 0 && a_offset < a_size);

    out->ptr[i] = a.ptr[static_cast<size_t>(a_offset)];

    // increment indices (carry)
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  for (size_t i = 0; i < a.size; i++) {
    // compute the offset in out
    size_t out_offset = offset;
    for (size_t d = 0; d < ndim; d++) {
      out_offset += indices[d] * strides[d];
    }
    out->ptr[out_offset] = a.ptr[i];
    // increment indices
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
  ...
  for (size_t i = 0; i < m / TILE; i++)
  {
    ...
    {
      memset(tmp, 0, sizeof(scalar_t) * TILE * TILE);
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for (size_t i = 0; i < out->size; i++) {
    ...
    for (size_t j = 1; j < reduce_size; j++) {
      scalar_t current_val = a.ptr[i * reduce_size + j];
