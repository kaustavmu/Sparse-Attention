...
class LSTMCell(Module):
    ...
    def forward(self, X, h=None):
        ...
        if h is None:
            ...
            h0, c0 = h
        if self.bias:
            gates = X @ self.W_ih + self.bias_ih.reshape((1, 4*self.hidden_size)).broadcast_to((batch_size, 4*self.hidden_size)) + \
        ...
        gates_all_split = tuple(ops.split(gates, axis = 1))
        gates = []
        for i in range(4):
...
class Embedding(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        ### BEGIN YOUR SOLUTION
        seq_len, bs = x.shape
        vocab_size, embed_dim = self.weight.shape
        x_one_hot = init.one_hot(
            vocab_size,
            x,
            device=self.device,
            dtype=self.dtype,
        ).reshape((seq_len * bs, vocab_size))

        if not self.use_sparse:
            embedded = x_one_hot @ self.weight
        else:
            sparse_one_hot = sops.dense_to_sparse(
                x_one_hot, threshold=self.one_hot_threshold, use_csr=True
            )
            sparse_weight = sops.dense_to_sparse(
                self.weight, threshold=0.0, use_csr=True
            )
            embedded_sparse = sops.sparse_matmul(sparse_one_hot, sparse_weight)
            embedded = sops.sparse_to_dense(embedded_sparse)

        return embedded.reshape((seq_len, bs, embed_dim))
        ### END YOUR SOLUTION